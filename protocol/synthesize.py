#!/usr/bin/env python3
"""
▌▌▌ THE XEROX PROJECT ▌▌▌
PHASE 1: SYNTHESIS

Extracts domain knowledge from source DNA and synthesizes training data.
The subject will consume this knowledge during duplication.
"""
from dotenv import load_dotenv
load_dotenv()

import json
import os
import sys
from pathlib import Path
from typing import Optional

# Add parent directory to path for baml_client imports
sys.path.insert(0, str(Path(__file__).parent.parent))

import anthropic
from baml_client.sync_client import b
from baml_client.types import AdvisorDomain, TrainingExample


# ============================================================================
# Domain Extraction
# ============================================================================

def extract_domain(system_prompt: str) -> AdvisorDomain:
    """Extract domain information from subject source DNA"""
    return b.ExtractDomain(system_prompt=system_prompt)


# ============================================================================
# Research Acquisition
# ============================================================================

def research_domain(client: anthropic.Anthropic, domain: AdvisorDomain) -> str:
    """Gather domain research via web search - the knowledge the subject will absorb"""

    search_prompt = f'''Research the following domain to gather expert knowledge:

DOMAIN: {domain.primary_domain}
EXPERTISE AREAS: {', '.join(domain.expertise_areas)}
KEY THEMES: {', '.join(domain.key_themes)}

Search for:
1. Expert advice and best practices in this domain
2. Common questions people ask about these topics
3. Research-backed strategies and frameworks
4. Real-world case studies or examples
5. Common misconceptions to address

For each finding, note:
- The source
- Key insights
- Practical advice given
- Questions being answered

Compile comprehensive research that could help an advisor in this domain.'''

    response = client.messages.create(
        model="claude-sonnet-4-20250514",
        max_tokens=8192,
        tools=[{
            "type": "web_search_20250305",
            "name": "web_search",
            "max_uses": 10
        }],
        messages=[{"role": "user", "content": search_prompt}]
    )

    research = ""
    for block in response.content:
        if hasattr(block, 'text'):
            research += block.text + "\n"

    return research


# ============================================================================
# Training Data Synthesis
# ============================================================================

def synthesize_training_data(
    domain: AdvisorDomain,
    research: str,
    system_prompt: str,
    num_examples: int = 100
) -> list[TrainingExample]:
    """Generate training examples - the curriculum the subject will learn"""

    batch_size = 25
    all_examples = []

    for i in range(0, num_examples, batch_size):
        current_batch = min(batch_size, num_examples - i)
        print(f"  Synthesizing batch {i//batch_size + 1} ({current_batch} examples)...")

        batch = b.SynthesizeTrainingData(
            name=domain.name,
            primary_domain=domain.primary_domain,
            expertise_areas=', '.join(domain.expertise_areas),
            communication_style=domain.communication_style,
            target_audience=domain.target_audience,
            research=research[:10000],
            system_prompt=system_prompt[:2000],
            num_examples=current_batch
        )
        all_examples.extend(batch)
        print(f"    Acquired {len(batch)} examples")

    return all_examples


# ============================================================================
# Output Formatting
# ============================================================================

def format_for_training(examples: list[TrainingExample], model_format: str = "mistral") -> list[str]:
    """Format examples for model consumption"""
    formatted = []

    for ex in examples:
        if model_format == "mistral":
            text = f"<s>[INST] {ex.instruction} [/INST] {ex.response}</s>"
        elif model_format == "llama":
            text = f"<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\n{ex.instruction}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n{ex.response}<|eot_id|>"
        else:
            text = f"User: {ex.instruction}\nAssistant: {ex.response}"

        formatted.append(text)

    return formatted


def save_training_data(examples: list[TrainingExample], output_dir: Path, subject_name: str):
    """Persist synthesized training data"""
    output_dir.mkdir(parents=True, exist_ok=True)

    # Raw JSON
    raw_path = output_dir / f"{subject_name}_raw.json"
    with open(raw_path, 'w') as f:
        json.dump([ex.model_dump() for ex in examples], f, indent=2)
    print(f"  Saved raw corpus: {raw_path}")

    # Mistral format JSONL
    mistral_path = output_dir / f"{subject_name}_mistral.jsonl"
    formatted = format_for_training(examples, "mistral")
    with open(mistral_path, 'w') as f:
        for text in formatted:
            f.write(json.dumps({"text": text}) + "\n")
    print(f"  Saved training data: {mistral_path}")

    return mistral_path


# ============================================================================
# Main Protocol
# ============================================================================

def run_synthesis(
    source_path: str,
    output_dir: str,
    num_examples: int = 500,
    api_key: Optional[str] = None
):
    """Execute the synthesis protocol"""

    api_key = api_key or os.environ.get("ANTHROPIC_API_KEY")
    if not api_key:
        raise ValueError("ANTHROPIC_API_KEY required - set in .env or pass --api-key")

    # Read source DNA
    with open(source_path) as f:
        system_prompt = f.read()

    client = anthropic.Anthropic(api_key=api_key)
    output_path = Path(output_dir)

    print()
    print("▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌")
    print("  THE XEROX PROJECT")
    print("  PHASE 1: SYNTHESIS")
    print("▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌")

    # Step 1: Extract domain from source DNA
    print("\n[1/4] Parsing source DNA...")
    domain = extract_domain(system_prompt)
    print(f"  Subject: {domain.name}")
    print(f"  Domain: {domain.primary_domain}")
    print(f"  Expertise: {', '.join(domain.expertise_areas)}")

    # Step 2: Research acquisition
    print("\n[2/4] Acquiring domain research...")
    research = research_domain(client, domain)
    print(f"  Gathered {len(research)} characters of research material")

    # Save research
    research_path = output_path / f"{domain.name.lower().replace(' ', '_')}_research.md"
    output_path.mkdir(parents=True, exist_ok=True)
    with open(research_path, 'w') as f:
        f.write(f"# Research Corpus: {domain.name}\n\n{research}")
    print(f"  Saved research corpus: {research_path}")

    # Step 3: Synthesize training data
    print(f"\n[3/4] Synthesizing {num_examples} training examples...")
    examples = synthesize_training_data(domain, research, system_prompt, num_examples)
    print(f"  Generated {len(examples)} examples")

    # Step 4: Save
    print("\n[4/4] Persisting training data...")
    subject_slug = domain.name.lower().replace(' ', '_').replace('.', '')
    training_path = save_training_data(examples, output_path, subject_slug)

    print()
    print("▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌")
    print("  SYNTHESIS COMPLETE")
    print("▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌")
    print(f"  Training corpus: {training_path}")
    print(f"  Examples: {len(examples)}")
    print()
    print("  Next phase - DUPLICATION:")
    print(f"    python protocol/duplicate.py --data {training_path}")
    print()

    return training_path


# ============================================================================
# CLI
# ============================================================================

if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(
        description="THE XEROX PROJECT - Phase 1: Synthesis",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Example:
  python protocol/synthesize.py \\
      --source subjects/001-elena-martinez/source.txt \\
      --output subjects/001-elena-martinez/training_data \\
      --examples 250
        """
    )
    parser.add_argument("--source", type=str, required=True,
                        help="Path to subject source DNA (system prompt)")
    parser.add_argument("--output", type=str, required=True,
                        help="Output directory for training data")
    parser.add_argument("--examples", type=int, default=500,
                        help="Number of training examples to synthesize")
    parser.add_argument("--api-key", type=str,
                        help="Anthropic API key (or set ANTHROPIC_API_KEY)")

    args = parser.parse_args()

    run_synthesis(
        source_path=args.source,
        output_dir=args.output,
        num_examples=args.examples,
        api_key=args.api_key
    )
